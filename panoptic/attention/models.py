...
import panoptic.attention.attention
from panoptic.attention.attention import (
    DecoderBlock,
    SemanticModule,
    MultiConv,
    GuidedAttention
)
from panoptic.attention.utils import get_ms_backbone, RemovePadding
...
def get_attention_head(key):
    return getattr(sys.modules[panoptic.attention.attention.__name__], f'{key}AttentionHead')


def get_attention_module(key):
    return getattr(sys.modules[panoptic.attention.attention.__name__], f'{key}Attention')
...
class AttentionMS(nn.Module):
    def __init__(self, in_channels=[256, 256, 256], base_channels=64,
        ...
        self.ups2 = nn.ModuleList([DecoderBlock(
            base_channels * (max(scales) - sc + 1),
            base_channels * (max(scales) - sc + 1)
        ) for sc in scales])
        self.ups1 = nn.ModuleList([DecoderBlock(
            base_channels * (max(scales) - sc + 1),
            base_channels * (max(scales) - sc + 1)
        ) for sc in scales])
